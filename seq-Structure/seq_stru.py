...
def load_and_fuse_structure_features(esm_features: np.ndarray, struct_emb_path: str = Config.STRUCTURE_EMB_PATH):
    if not os.path.exists(struct_emb_path):
        raise FileNotFoundError(f"Structural feature file not found: {struct_emb_path}")
    struct_features = np.load(struct_emb_path)
    
    if struct_features.shape[0] != esm_features.shape[0]:
        raise ValueError(
            f"Sample count mismatch: ESM features {esm_features.shape[0]} samples, structural features {struct_features.shape[0]} samples"
        )
    
    if len(struct_features.shape) != 2 or struct_features.shape[1] != Config.STRUCTURE_DIM:
        raise ValueError(
            f"Structural feature shape should be (N, {Config.STRUCTURE_DIM}), actual is {struct_features.shape}"
        )
    
    struct_features_expanded = np.repeat(
        struct_features[:, np.newaxis, :],
        repeats=esm_features.shape[1],
        axis=1
    )
    
    fused_features = np.concatenate([esm_features, struct_features_expanded], axis=2)
    print(f"Feature fusion completed: "
          f"ESM features {esm_features.shape} + "
          f"structural features {struct_features.shape} → "
          f"fused features {fused_features.shape}")
...
def main():
    ...
    parser.add_argument('--struct_emb', type=str, default=Config.STRUCTURE_EMB_PATH, help='Path to structural feature file')
    ...
    print(f"ESM feature shape: {X_esm.shape} (should be N×31×2560)")

    X = load_and_fuse_structure_features(X_esm, args.struct_emb)
    ...
    if args.mode == 'train':
        ...
        print("\n=== Single fold training final metrics ===")
    ...
    if args.mode == 'kfold':
        ...
        for fold_idx, (train_idx, val_idx) in enumerate(skf.split(X, labels), 1):
            ...
            print(f"Fold {fold_idx} metrics summary:")
