...
def read_seqs_from_csv(pos_csv, neg_csv, seq_col = 'Fragment'):
...
def load_esm2_model(model_name, device: torch.device):
...
def pad_features(features, maxlen):
...
def optuna_objective(trial, X, y, args):
    ...
    for train_idx, val_idx in skf.split(X, y):
        ...
        model, history = train_one_fold(model, tr_loader, v_loader, Config.DEVICE, epochs=8, lr=lr, weight_decay=weight_decay, clip_norm=Config.CLIP_NORM, label_smoothing_eps=Config.LABEL_SMOOTH_EPS, patience=3, use_focal=Config.USE_FOCAL)
...
def main():
    ...
    print('ESM2 features shape:', X.shape)
    ...
    if args.mode == 'train':
        ...
        model = ESM2_CNN_BiLSTM(seq_len=X.shape[1], esm2_dim=X.shape[2]).to(Config.DEVICE)
        if Config.USE_FOCAL:
        ...
        print("\n=== Results ===")
        print(f"Train set - AUC: {all_metrics['train_metrics']['auc']:.4f}, Loss: {all_metrics['train_metrics']['loss']:.4f}")
        print(f"Val set - AUC: {all_metrics['val_metrics']['auc']:.4f}, F1: {all_metrics['val_metrics']['f1']:.4f}, MCC: {all_metrics['val_metrics']['mcc']:.4f}")
    ...
    if args.mode == 'kfold':
        ...
        for fold_idx, (train_idx, val_idx) in enumerate(skf.split(X, labels), 1):
            ...
            print(f"  Train AUC: {fold_metrics['train_metrics']['auc']:.4f}, Val AUC: {fold_metrics['val_metrics']['auc']:.4f}, Test AUC: {fold_metrics['test_metrics']['auc']:.4f}")
        ...
        print("\n=== The average indicator of K-fold cross-validation ===")
        print(f"Fold: {args.n_splits}")
        print(f"Best Fold: {best_fold_idx} (Val AUC: {best_val_auc:.4f})")
        print(f"Mean train AUC: {avg_metrics['avg_train_auc']:.4f}")
        print(f"Mean val AUC: {avg_metrics['avg_val_auc']:.4f}")
        print(f"Mean test AUC: {avg_metrics['avg_test_auc']:.4f}")
        print(f"Mean test F1: {avg_metrics['avg_test_f1']:.4f}")
        print(f"Mean test MCC: {avg_metrics['avg_test_mcc']:.4f}")
        ...
        best_model_path = os.path.join(model_dir, 'best_fold_model.pth')
        best_fold_model_path = os.path.join(model_dir, f'best_model_fold{best_fold_idx}.pth')
